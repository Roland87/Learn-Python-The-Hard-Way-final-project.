...
"""
The Gotthns of Planet Percal #25 have invaded your ship and destroyed
...
the Armory and about to pull a wepon to blast you.
""")
...
to not laugh, then busts out laughing and can't move. While he's
...
more Gothons that might be hiding. It's dead quite, too quite. You
stand up and run to the far side of the room and find the neutron bomb
in its cintainer. There's a keypad lock on the box and you need the
code to get the bomb out. If you get the code wrong 10 times then the
lock closes forever and you can't get the bomb. Code is 3 digits.
""")
...
You burst onto the Bridge with the neutron destruct bomb under yor arm
and suprise 5 Gothons who are trying to tak control of the ship. Each
...
"""
You poit your blaster at the bomb under your arm and the Gothons put
their hands up and start to sweat. You inch backward to the door, open
it, and then carfeully place the bomb on the floor, pointing your
blaster at it. You then jump back through the door, punch the close
button and blast the lock so the Gothons can't get out. Now thet the
...
them coud be damaged but you don't have time to look. There is 5 pods,
witch one you take?
""")
...
look back and see your ship implode then expode like a bright star,
tanking out the Gothon ship at the same time. You won!
""")
...
out into the void space, then implodes as th hull ruptures, crushing
...
def load_room(name):
    ...
    Who gets to set name? Can that expose a varible?
    """
    return globals().get(name)

def name_room(room):
    """
    Same pssible security problem. Can you trust room?
